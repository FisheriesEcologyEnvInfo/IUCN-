...
fishData2 = fishDataCountries %>%                     
  group_by(ISO2) %>%                        #grouping by ISO2 column
  summarize(                                #calculating the mean and rounded to 1 digit
    mean = round(mean(index),digits=1))
...
fishData2$levels <- levels(fishData2$mean)  #Creating display leyend levels for each country in the map

suppressWarnings(suppressMessages(library(googleVis))) #install.packages("googleVis")
#### Create a geo chart
G1 <- gvisGeoChart(fishData2, "ISO2", "mean", hovervar="ISO2",
