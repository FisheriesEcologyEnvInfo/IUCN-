...
**Table I-** Dietary items from **P. volitans**



```{r, echo=FALSE, fig.cap="Length - Weight relationship for *P. volitans*"}

ggplot(data=pvol, aes(x=TotalLength_cm, y=TotalWeigth_gr, col=Location))+
  geom_point()+
  theme_bw()+
  labs(x="Total length (cm)", y="Total Weight (gr)")
...
```{r, echo=FALSE, fig.cap="Tuna catches made by the Mexican Tuna Purse Seine Fleet in 2013 and 2014"}
...
summary = tuna %>%
  ...
  group_by(Type, Year) %>%
  summarize(mean_length_cm=mean(Length_cm), catch_t=sum(Catch_t))
```

**Table II-** Mean size and total catch per type of cast. LANMAM = casts with tuna and dolphins, LANATUN = casts on free swimming tuna, LANPALO = casts ontuna with FADS.
