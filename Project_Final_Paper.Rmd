...
We created two different Shiny Applications. The first one represents ** JC **

![Figure 4](images/fish-ecol pic.png)


Do we need the code below? ** JC **
```{r, TaxApp, eval=FALSE}
library(shiny)
...
The function `fish.ecol::Extantify()` was created to address this need as a data extraction tool for simple usage. It wrapped a number of data needs into a single function such as removing presence data outside of our scope (regionally extinct species or uncertain distributions) and non-coastal countries in the output. Additionally, it provided a flexible framework for subsetting the data output to reduce the number of code lines needed in our analyses.

`fish.ecol::iucn_index()` ** JC **
...
```{r IUCN Taxonomic Graphs,echo=FALSE,fig.width=9, fig.height=6}
...
CategoryGraph=ggplot(data=SummaryTable1,aes(x=Class,y=Count,fill=GlobalStatus,order=GlobalStatus,width=.3))+
    ...
    ggtitle(NULL)+
    theme_bw()+
    ggtitle("Species Category Percentages By Class")+
